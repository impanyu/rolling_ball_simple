...
export default function SimulatorPage() {
    ...
    const handlePChange = async (newPA: number, newPB: number) => {
        ...
        if (lookup) {
            setSimulating(true);
            try {
                const sim = await runSimulation(newPA, newPB, lookup.current_score, firstServer, 10000);
                setSimResult(sim);
            } catch (err) {
    ...
    const thresholdA = probA != null ? Math.min(Math.floor((probA + 5) / 5) * 5, 95) : null;
    const thresholdB = probB != null ? Math.min(Math.floor((probB + 5) / 5) * 5, 95) : null;
        ...
        <div>
                    ...
                    <button
                        ...
                        onClick={async () => {
                            ...
                            try {
                                ...
                                if (lookup) {
                                    const sim = await runSimulation(pA, pB, lookup.current_score, firstServer, 10000);
                                    setSimResult(sim);
                                }
                            } catch (err) {
                    ...
                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                        ...
                        <button
                            onClick={async () => {
                                setFirstServer("a");
                                if (lookup && simResult) {
                                    const sim = await runSimulation(pA, pB, lookup.current_score, "a", 10000);
                                    setSimResult(sim);
                                }
                            }}
                            style={{
                        ...
                        <button
                            onClick={async () => {
                                setFirstServer("b");
                                if (lookup && simResult) {
                                    const sim = await runSimulation(pA, pB, lookup.current_score, "b", 10000);
                                    setSimResult(sim);
                                }
                            }}
                            style={{
            ...
            <div style={{ marginTop: 16 }}>
                ...
                {simTab === "maxprob" && maxResult && (() => {
                    ...
                    const threshold = Math.min(Math.floor(((currentProb ?? 50) + 5) / 5) * 5, 95);
